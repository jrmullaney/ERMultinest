...
def myloglike(cube, ndim, nparams):
    ...
    #int2 = abs(quad(integrand, 1e-3, np.inf, args = alpha)[0])
    int2 = 10**poly.polyval(alpha, pars)
    #print(int/int2)
    lik2 = k*int2
...
#Create a polynomial fit for alpha:
...
#Run MultiNest
pymultinest.run(myloglike, myprior, n_params, importance_nested_sampling = False, resume = False, verbose = True, sampling_efficiency = 'model', n_live_points = 500, outputfiles_basename='chains/PL_SFR-')

#Analyse the results and plot the marginals
ana = pymultinest.Analyzer(n_params = n_params, outputfiles_basename='chains/PL_SFR-')
